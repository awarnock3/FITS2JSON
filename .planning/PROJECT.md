...
FITS2JSON is a command-line tool for reading FITS headers and emitting them as structured JSON instead of raw terminal text. It is aimed at scriptable workflows that need machine-readable header metadata while staying close to the existing `src/listhead.c` CFITSIO-based implementation.
...
- [ ] User can use the tool in shell scripts with deterministic output and failure behavior
...
The immediate feature direction is to keep the existing FITS traversal behavior and change the output layer from raw card dumps to structured JSON. The user explicitly wants `src/listhead.c` used as the starting point and expects the implementation to lean on CFITSIO knowledge rather than introducing a new parsing stack.
...
- **Interface**: CLI stdout output — the tool is intended for shell-script consumption, not an interactive UI
